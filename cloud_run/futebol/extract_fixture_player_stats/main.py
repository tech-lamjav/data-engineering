...
# pra manter o import `from extract_fixture_player_stats import main` simples.
...
from extract_fixture_player_stats import main as extract_main


@functions_framework.http
def extract_fixture_player_stats(request):
    """API-Football /fixtures/players pipeline.
    ...
    try:
        ...
        # O script lê o modo de FIXTURE_PLAYER_STATS_MODE dentro do main(). Setamos a env var
        ...
        _prev_mode = os.environ.get("FIXTURE_PLAYER_STATS_MODE")
        os.environ["FIXTURE_PLAYER_STATS_MODE"] = mode
        try:
            result_code = extract_main()
        finally:
            if _prev_mode is None:
                os.environ.pop("FIXTURE_PLAYER_STATS_MODE", None)
            else:
                os.environ["FIXTURE_PLAYER_STATS_MODE"] = _prev_mode
        if result_code == 0:
