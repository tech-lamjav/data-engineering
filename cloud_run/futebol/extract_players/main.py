...
# pra manter o import `from extract_players import main` simples.
...
from extract_players import main as extract_main


@functions_framework.http
def extract_players(request):
    """API-Football /players pipeline.
    ...
    try:
        ...
        # O script lê o modo de PLAYERS_MODE dentro do main(). Setamos a env var
        ...
        _prev_mode = os.environ.get("PLAYERS_MODE")
        os.environ["PLAYERS_MODE"] = mode
        try:
            result_code = extract_main()
        finally:
            if _prev_mode is None:
                os.environ.pop("PLAYERS_MODE", None)
            else:
                os.environ["PLAYERS_MODE"] = _prev_mode
        if result_code == 0:
