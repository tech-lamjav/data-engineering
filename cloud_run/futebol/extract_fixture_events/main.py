...
def extract_fixture_events(request):
    ...
    try:
        ...
        # O script lê o modo de FIXTURE_EVENTS_MODE dentro do main(). Setamos a env var
        ...
        _prev_mode = os.environ.get("FIXTURE_EVENTS_MODE")
        os.environ["FIXTURE_EVENTS_MODE"] = mode
        try:
            result_code = extract_main()
        finally:
            if _prev_mode is None:
                os.environ.pop("FIXTURE_EVENTS_MODE", None)
            else:
                os.environ["FIXTURE_EVENTS_MODE"] = _prev_mode
        if result_code == 0:
