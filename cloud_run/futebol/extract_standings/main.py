...
# pra manter o import `from extract_standings import main` simples.
...
from extract_standings import main as extract_main


@functions_framework.http
def extract_standings(request):
    """API-Football /standings pipeline (snapshot diário da tabela do campeonato).
    ...
    try:
        ...
        # O script lê o modo de STANDINGS_MODE dentro do main(). Setamos a env var
        ...
        _prev_mode = os.environ.get("STANDINGS_MODE")
        os.environ["STANDINGS_MODE"] = mode
        try:
            result_code = extract_main()
        finally:
            if _prev_mode is None:
                os.environ.pop("STANDINGS_MODE", None)
            else:
                os.environ["STANDINGS_MODE"] = _prev_mode
        if result_code == 0:
