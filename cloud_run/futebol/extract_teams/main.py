...
# pra manter o import `from extract_teams import main` simples.
...
from extract_teams import main as extract_main


@functions_framework.http
def extract_teams(request):
    """API-Football /teams pipeline.
    ...
    try:
        ...
        # O script lê o modo de TEAMS_MODE dentro do main(). Setamos a env var
        ...
        _prev_mode = os.environ.get("TEAMS_MODE")
        os.environ["TEAMS_MODE"] = mode
        try:
            result_code = extract_main()
        finally:
            if _prev_mode is None:
                os.environ.pop("TEAMS_MODE", None)
            else:
                os.environ["TEAMS_MODE"] = _prev_mode
        if result_code == 0:
