...
def test_so_a_janela_diaria_date_stampa():
    assert FUTEBOL_ODDS_WINDOWS_DIARIAS == {"daily"}
...
def test_sem_odds_publicadas_nao_grava(ext):
    # Odds NAO grava vazio (diferente de injuries): a banda de fechamento e forward-only
    # e precisa re-tentar no proximo poll ate a casa publicar.
    ext.storage.get_upcoming_fixtures_with_kickoff.return_value = [_fixture(72)]
