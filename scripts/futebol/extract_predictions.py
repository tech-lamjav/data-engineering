...
Sem argparse (regra `.cursorrules`). Sem modo: uma passada cobre a janela T-2h — o
extractor bucketa cada jogo NS pela proximidade do kickoff. Forward-only. Rodado pelo
poll workflow_futebol_predictions.yml (Cloud Scheduler ~15min).
"""
import sys
...
def main():
    try:
        logger.info("Iniciando extract_predictions (janela T-2h)")
