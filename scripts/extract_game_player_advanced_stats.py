"""Script individual para extração de estatísticas avançadas (tracking/passing) por jogo."""
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))

from src.extractors.game_player_advanced_stats_extractor import GamePlayerAdvancedStatsExtractor
...
def main():
    try:
        ...
        for season in _seasons:
            logger.info(f"Iniciando extração de estatísticas avançadas para temporada {season}")
            gcs_paths = GamePlayerAdvancedStatsExtractor(season=season).extract_and_save()
